...
void entry_clicked(GtkWidget *widget, gpointer data) {
	...
	GtkWidget *button;
	GtkWidget *fixed;
	gchar *names[] = { "White Player Name:", "White Player Age:",
			"Black Player Name:", "Black Player Age:", "Yellow Player Name:",
			"Yellow Player Age:" };
...
//				g_strdup_printf("Player No.%i's name:", i + 1));
	...
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(start_clicked),
			(void * )entries);
